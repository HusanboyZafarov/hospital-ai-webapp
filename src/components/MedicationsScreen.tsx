...
export function MedicationsScreen({ onViewSchedule }: MedicationsScreenProps) {
  const [medications, setMedications] = useState([
    { id: 1, name: 'Amoxicillin', dosage: '500mg', time: '8:00 AM', status: 'taken' },
    { id: 2, name: 'Ibuprofen', dosage: '400mg', time: '2:00 PM', status: 'upcoming' },
    { id: 3, name: 'Vitamin D', dosage: '1000 IU', time: '8:00 PM', status: 'upcoming' },
  ]);
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'taken': return 'var(--success-green)';
      case 'missed': return 'var(--error-red)';
            ...
            {medications.map((med) => (
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px' }}>
                  ...
                  <p style={{ color: 'var(--muted-text)' }}>Due at {med.time}</p>
                </div>
              ...
              <div key={index} style={{ textAlign: 'center', minWidth: '60px' }}>
                <div 
                  style={{
                    ...
                    height: '40px',
                    borderRadius: '10px',
                    backgroundColor: index === 0 ? 'var(--primary-blue)' : 'var(--card-grey)',
                    display: 'flex',
                    alignItems: 'center',
                ...
                >
                  <div 
                    style={{
                      ...
                      height: '20px',
                      borderRadius: '50%',
                      backgroundColor: index === 0 ? 'white' : 'var(--muted-text)',
                    }}
                  />
                </div>
                <p className="caption" style={{ color: 'var(--muted-text)' }}>{hour}</p>
              </div>
            ))}
          </div>
        </div>
