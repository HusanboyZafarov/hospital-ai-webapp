...
export function CarePlanScreen({ onBack }: CarePlanScreenProps) {
  const [expandedSections, setExpandedSections] = useState<string[]>(['pre-op']);
      ...
      <div style={{ padding: '16px' }}>
            ...
            return (
              ...
              >
                ...
                >
                  <h3>{section.title}</h3>
                  {isExpanded ? (
                    <ChevronUp size={20} style={{ color: 'var(--primary-blue)' }} />
                  ) : (
                    <ChevronDown size={20} style={{ color: 'var(--muted-text)' }} />
                  )}
                </button>
                  ...
                  <div style={{ padding: '0 16px 16px' }}>
                    <ul style={{ 
                      ...
                      {section.items.map((item, index) => (
                        <li key={index} style={{ color: 'var(--dark-text)' }}>{item}</li>
                      ))}
                    </ul>
                  </div>
                )}
        ...
        <div style={{
          ...
          <p style={{ color: 'var(--dark-text)' }}>
            If you experience severe pain, difficulty breathing, or signs of infection, call your doctor immediately or go to the nearest emergency room.
          </p>
