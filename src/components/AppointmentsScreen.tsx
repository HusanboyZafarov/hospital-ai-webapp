...
export function AppointmentsScreen() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'Confirmed': return 'var(--success-green)';
      ...
      <div style={{ padding: '16px' }}>
        ...
        <div style={{
          ...
          <div style={{ 
            ...
            <p className="caption" style={{ color: 'white' }}>Next Appointment</p>
          </div>
          
          <h3 style={{ marginBottom: '16px', color: 'white' }}>{upcomingAppointment.type}</h3>
              ...
              >
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    ...
                    <p style={{ color: 'var(--muted-text)' }}>{appointment.date}</p>
                  </div>
                  
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <Clock size={16} style={{ color: 'var(--muted-text)' }} />
                    <p style={{ color: 'var(--muted-text)' }}>{appointment.time}</p>
                  </div>
                  
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <User size={16} style={{ color: 'var(--muted-text)' }} />
                    <p style={{ color: 'var(--muted-text)' }}>{appointment.doctor}</p>
                  </div>
                </div>
              </div>
            ))}
